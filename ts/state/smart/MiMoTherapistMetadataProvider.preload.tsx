...
function getSingleRemoteGroupRecipientServiceId(
  call: ActiveCallType | undefined
  ...
  const candidate = remote?.serviceId ?? remote?.aci ?? '';
  return normalizeServiceId(
    candidate,
    'MiMoTherapistMetadataProvider.groupFallback'
  );
}
  ...
  function MiMoTherapistMetadataProvider({
    ...
    useEffect(() => {
      ...
      void (async () => {
        ...
        const fallbackTherapist = getSingleRemoteGroupRecipientServiceId(
          activeCallRef.current
        );
        const therapistServiceId: ServiceIdString | null =
          normalizedTherapist ?? fallbackTherapist;

        if (!normalizedTherapist && fallbackTherapist) {
          log.info(
            'MiMo group: using single-remote fallback recipient (set MIMO_THERAPIST_SERVICE_ID to pin therapist target)'
          );
        }

        if (canceled || therapistServiceId == null) {
          if (!canceled && groupMonitorKey != null) {
            log.info(
              'MiMo group: skipping metadata (set MIMO_THERAPIST_SERVICE_ID, or ensure exactly one remote participant for fallback)'
            );
          }
          return;
        }
