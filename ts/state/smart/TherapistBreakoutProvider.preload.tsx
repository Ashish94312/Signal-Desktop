...
const BREAKOUT_DIRECT_INVITE_GRACE_MS = 6000;
    ...
    const handleBreakoutToOneToOne = useCallback(
      async (
        ...
        if (!recipientServiceId) {
          log.warn(
            'TherapistBreakoutProvider: missing recipient serviceId; aborting breakout'
          );
          return;
        }
...
export function useTherapistBreakout(): TherapistBreakoutContextValueType {
  ...
  if (!ctx) {
    throw new Error('useTherapistBreakout must be used within TherapistBreakoutProvider');
  }
  return ctx;
}
