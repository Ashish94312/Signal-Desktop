...
/**
 * Suppress the standard Signal call manager when the therapist console is
 * open — the console handles all call UI (incoming banner, active call view,
 * video tiles) and both rendering the same call simultaneously causes
 * video-stream conflicts and duplicate call controls.
 *
 * Always renders a floating "Console" button so the therapist can navigate
 * back to the console both during a call and when no call is active.
 */
const SmartConditionalCallManager = memo(
  function SmartConditionalCallManager() {
    const isTherapistConsoleVisible = useSelector(
      (state: { globalModals: { isTherapistConsoleVisible: boolean } }) =>
        state.globalModals.isTherapistConsoleVisible
    );
    const { showTherapistConsole } = useGlobalModalActions();

    return (
      <>
        {!isTherapistConsoleVisible && (
          <button
            aria-label="Open therapist console"
            type="button"
            onClick={showTherapistConsole}
            style={{
              position: 'fixed',
              bottom: '16px',
              left: '16px',
...
function renderCallManager(): React.JSX.Element {
  return <SmartConditionalCallManager />;
}
