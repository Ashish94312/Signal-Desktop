...
import type { SetRendererCanvasType, StartCallType } from '../state/ducks/calling.preload.ts';
...
type PropsType = {
  ...
  /** Incoming direct Signal call ringing — shown as a banner so the therapist can accept without leaving the console. */
  incomingCall?: {
    conversationId: string;
    isVideoCall: boolean;
    callerName?: string;
  } | null;
  onAcceptCall?: (asVideoCall: boolean) => void;
  onDeclineCall?: () => void;
};
...
export function TherapistConsoleModal({
  ...
  const [editingCallLinkRoomId, setEditingCallLinkRoomId] = useState<string | null>(null);
  ...
  const mimoClients = useSelector((state: StateType) => state.mimoSession.clients);
  ...
  const groupMembershipsForAddMembers = useMemo(() => {
    if (!selectedLaunchConversation || selectedLaunchConversation.type !== 'group') {
      return null;
    }
  ...
  const sessions = useMemo<Array<ConsoleSession>>(() => {
    ...
    if (activeCall.callMode === CallModeValue.Direct) {
      ...
      if (!activeCall.hasRemoteVideo) {
        cues.push({ label: 'Video', detail: 'Remote video unavailable.', tone: 'warning' });
      }
      if (!activeCall.hasRemoteAudio) {
        cues.push({ label: 'Audio', detail: 'Remote audio unavailable.', tone: 'warning' });
      }
      if (activeCall.callState === CallState.Reconnecting) {
        cues.push({ label: 'Connection', detail: 'Participant reconnecting.', tone: 'warning' });
      }
    ...
    return activeCall.remoteParticipants.map(participant => {
      ...
      if (participant.presenting) {
        cues.push({ label: 'Presentation', detail: 'Currently presenting.', tone: 'info' });
      }
      if (isSpeaking) {
        cues.push({ label: 'Activity', detail: 'Speaking now.', tone: 'good' });
      }
      ...
      if (!participant.mediaKeysReceived) {
        cues.push({ label: 'Media Keys', detail: 'Media keys have not arrived yet.', tone: 'warning' });
      }
  ...
  const selectedInterventionTargetTitle =
    selectedInterventionConversation?.title ?? selectedSession?.title ?? 'this session';
  ...
  }, [activeCall, isSendingVideo, setLocalPreviewContainer, shouldShowStageLocalPreview]);
  ...
  // Size callback for the direct-call remote video canvas.
  // The canvas fills its container via CSS; we don't need to track PiP width here.
  const handleDirectVideoSize = useCallback<SizeCallbackType>(() => {
    // no-op: canvas is sized by CSS (width/height: 100%)
  }, []);
      ...
      <div style={csMiMoPanel()}>
        ...
        >
          ...
          >
            ...
            Activity Monitor
            {totalMimoAlerts > 0 ? (
              <span style={{ ...csBadge('neutral'), padding: '2px 8px', fontSize: '10px' }}>
        ...
        {!isActivityMonitorCollapsed ? (
          <>
            {mimoClientList.length === 0 ? (
              <div style={{ fontSize: '12px', color: CS.onSurfaceMuted, lineHeight: 1.5 }}>
                No client activity detected yet.
              </div>
            ) : (
              <div style={{ display: 'grid', gap: '8px' }}>
                {mimoClientList.map(client => {
                  const match = findMiMoCallMatch(activeCall, client.clientSessionId);
                  const hasRed = client.alerts.some(a => a.severity === 'red');
                  const hasYellow = client.alerts.some(a => a.severity === 'yellow');
                  const dotColor = hasRed
                    ...
                    : hasYellow
                      ...
                      : match.kind === 'participant'
                        ...
                        : CS.warning;
                  const rail =
                    hasRed ? CS.critical : hasYellow ? CS.warning : 'transparent';
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        {client.alerts.length > 0 ? (
                          <span
                            style={{
                              ...csBadge(
                                hasRed ? 'critical' : hasYellow ? 'warning' : 'good'
                              ),
                              padding: '2px 8px',
                              fontSize: '10px',
                              flexShrink: 0,
                      ...
                      {client.sessionStatus ? (
                        <div style={{ color: CS.onSurfaceMuted, fontSize: '11px' }}>
            ...
            {!selectedMimoSnapshot ? (
              <div style={{ fontSize: '12px', color: CS.onSurfaceMuted, lineHeight: 1.5 }}>
                Select a session tile in the console to correlate MiMo metadata with a
                participant.
              </div>
              ...
              >
                ...
                <div style={{ fontSize: '12px', color: CS.onSurfaceMuted }}>
                  This client is stable according to the latest metadata for the selected
                  session.
                </div>
              </div>
            ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <span style={{ ...csBadge(severityTone), padding: '4px 10px', fontSize: '10px' }}>
                        ...
                        >
                          Triggered {formatRelativeTimeForMimoAlert(alert.timestamp)}
                        </div>
                      </article>
                    );
                  })}
  ...
  }, [activeCall, mimoClientList, selectedMimoSnapshot, isActivityMonitorCollapsed]);
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              {activeCall ? (
                ...
                >
                  ...
                  />
                  <span style={{ fontSize: '12px', fontWeight: 650, color: CS.success }}>
                    Session Active
                  </span>
                </div>
              ) : (
                ...
                >
                  <span style={{ ...csStatusDot(CS.onSurfaceMuted), width: '8px', height: '8px' }} />
                  <span style={{ fontSize: '12px', fontWeight: 600, color: CS.onSurfaceMuted }}>
                    Ready
                  </span>
                </div>
              )}
          ...
          {incomingCall ? (
            <div
              style={{
                ...
                gap: '12px',
                padding: '12px 20px',
                background: 'rgba(36, 59, 120, 0.08)',
                borderBottom: `1px solid rgba(36, 59, 120, 0.18)`,
                flexShrink: 0,
              }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: '11px', fontWeight: 600, color: '#243b78', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                    Incoming {incomingCall.isVideoCall ? 'video' : 'audio'} call
                  </div>
                  {incomingCall.callerName ? (
                    <div style={{ fontSize: '13px', color: '#1d2433', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {incomingCall.callerName}
                    </div>
                  ) : null}
                </div>
              ...
              <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                {onDeclineCall ? (
                  <button
                    ...
                    style={{
                      padding: '6px 14px',
                      borderRadius: CS.radiusFull,
                      border: '1px solid rgba(192, 68, 68, 0.35)',
                ...
                {onAcceptCall ? (
                  <button
                    type="button"
                    onClick={() => onAcceptCall(incomingCall.isVideoCall)}
                    style={{
                      padding: '6px 14px',
                      borderRadius: CS.radiusFull,
                      border: '1px solid rgba(36, 59, 120, 0.35)',
                      background: '#243b78',
                      color: '#ffffff',
                      fontSize: '13px',
                      fontWeight: 600,
              ...
              <aside style={csAside()}>
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                    ...
                    >
                      <Icon svg={ICONS.heart} size={18} color={CS.secondary} />
                    </div>
                    <div>
                      <div style={{ fontSize: '14px', fontWeight: 700, color: CS.onSurface, fontFamily: CS.fontDisplay }}>
                        Welcome back
                      </div>
                      <div style={{ fontSize: '11px', color: CS.onSurfaceMuted, marginTop: '1px' }}>
                        {new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' })}
                      </div>
                    </div>
                  </div>
                  <div style={{ fontSize: '12px', color: CS.onSurfaceVariant, lineHeight: 1.5 }}>
                    Select a client or call link to begin a session.
                  </div>
                </div>

                {/* Nav tabs */}
                <div style={{ ...csSectionLabel(), paddingLeft: '14px', marginBottom: '6px' }}>
                  Navigation
                </div>
                <nav style={{ display: 'flex', flexDirection: 'column', gap: '2px', marginBottom: '20px' }}>
                  {shellTabs.map(tab => {
                    ...
                    return (
                      ...
                      >
                        <span style={csNavTabIcon(isSelected)} dangerouslySetInnerHTML={{ __html: TAB_ICONS[tab.id] }} />
                        {tab.label}
                      </button>
                    );
                  })}
              ...
              <div style={csMainColumn()}>
                ...
                {activeShellTab === 'workspace' ? (
                  <div style={{ display: 'grid', gap: '24px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <div>
                        <h2 style={{ ...csWorkspaceTitle(), marginBottom: '4px' }}>Sessions</h2>
                        <p style={{ margin: 0, fontSize: '13px', color: CS.onSurfaceMuted, lineHeight: 1.4 }}>
                    ...
                    >
                      ...
                      <div style={{ ...csCard(), padding: '16px' }}>
                        <div style={{ ...csSectionLabel(), marginBottom: '12px', paddingLeft: '6px' }}>
                          Clients &amp; Groups
                        </div>
                        <div style={{ display: 'grid', gap: '3px' }}>
                          {launchableConversations.length ? (
                            launchableConversations.map(conversation => {
                              const isSelected = selectedLaunchConversationId === conversation.id;
                              return (
                                <button
                                  key={conversation.id}
                                  type="button"
                                  onClick={() => setSelectedLaunchConversationId(conversation.id)}
                                  style={{
                                ...
                                >
                                  <AvatarCircle name={conversation.title} size={34} />
                                  <div style={{ minWidth: 0, flex: 1 }}>
                                    <div
                                      style={{
                                        ...
                                        whiteSpace: 'nowrap',
                                        fontSize: '13px',
                                        color: isSelected ? CS.onSurface : CS.onSurfaceSecondary,
                                      }}
                                    >
                                      {conversation.title}
                                    </div>
                                  ...
                                  {isSelected && (
                                    <Icon svg={ICONS.chevronRight} size={13} color={CS.primary} />
                                  )}
                                </button>
                              );
                            })
                            ...
                            >
                              <Icon svg={ICONS.users} size={24} color={CS.onSurfaceMuted} style={{ marginBottom: '8px', opacity: 0.5 }} />
                              <div style={{ fontSize: '13px', color: CS.onSurfaceMuted, lineHeight: 1.5 }}>
                                No clients yet.
                              </div>
                              <div style={{ fontSize: '11px', color: CS.onSurfaceMuted, marginTop: '4px' }}>
                          ...
                          {selectedLaunchConversation ? (
                            <>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '18px' }}>
                                <AvatarCircle name={selectedLaunchConversation.title} size={44} />
                                <div style={{ flex: 1, minWidth: 0 }}>
                                  ...
                                  <div style={{ fontSize: '12px', color: CS.onSurfaceMuted, marginTop: '3px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                    <span style={csBadge('neutral')}>
                                      {workspaceConversationType}
                                    </span>
                                    {workspaceConversationMembers > 0 ? (
                                      <span style={{ color: CS.onSurfaceMuted }}>{workspaceConversationMembers} members</span>
                                    ) : null}
                                    <span style={{ color: CS.outlineVariant }}>·</span>
                                    <span>{workspaceConversationLastActive}</span>
                                  </div>
                                </div>
                              </div>
                              ...
                              {/* Group members */}
                              {groupMembershipsForAddMembers &&
                              groupMembershipsForAddMembers.memberships.length > 0 ? (
                                <div style={{ marginBottom: '18px' }}>
                                  <div style={{ ...csSectionLabel(), marginBottom: '10px' }}>
                                    Participants ({groupMembershipsForAddMembers.memberships.length})
                                  </div>
                                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                    {groupMembershipsForAddMembers.memberships.map(
                                      membership => (
                                        ...
                                        >
                                          <AvatarCircle name={membership.member.title} size={20} />
                              ...
                              >
                                <button
                                  type="button"
                                  disabled={selectedLaunchConversation.type !== 'direct'}
                                  onClick={() => onStartAudioCall(selectedLaunchConversation.id)}
                                  style={{
                                    ...(selectedLaunchConversation.type === 'direct'
                                      ? csSecondaryButton()
                                      : csPrimaryButton(true)),
                                    display: 'inline-flex',
                                    alignItems: 'center',
                                ...
                                <button
                                  type="button"
                                  onClick={() => onStartVideoCall(selectedLaunchConversation.id)}
                                  style={{
                                ...
                                >
                                  ...
                                  {selectedLaunchConversation.type === 'group' ? 'Start Session' : 'Video Call'}
                                </button>
                              </div>
                            </>
                          ) : (
                            ...
                            >
                              ...
                              >
                                <Icon svg={ICONS.video} size={22} color={CS.onSurfaceMuted} />
                              </div>
                              <div style={{ fontSize: '14px', fontWeight: 600, color: CS.onSurfaceSecondary, marginBottom: '4px' }}>
                                Select a client
                              </div>
                              <div style={{ fontSize: '12px', color: CS.onSurfaceMuted, lineHeight: 1.5 }}>
                                Choose someone from the list to start a session.
                              </div>
                            </div>
                          )}
                        </div>
                        ...
                        <div style={csCard()}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', marginBottom: '16px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              ...
                              >
                                <Icon svg={ICONS.link} size={14} color={CS.primary} />
                              </div>
                              <span style={{ fontSize: '14px', fontWeight: 700, color: CS.onSurface, fontFamily: CS.fontDisplay }}>
                                Call Links
                              </span>
                            </div>
                            <button
                            ...
                            <div style={{ display: 'grid', gap: '4px' }}>
                              {launchableCallLinks.length ? (
                                launchableCallLinks.map(callLink => {
                                  const isSelected = selectedLaunchCallLinkRoomId === callLink.roomId;
                                  return (
                                    <button
                                      key={callLink.roomId}
                                      type="button"
                                      onClick={() => setSelectedLaunchCallLinkRoomId(callLink.roomId)}
                                      style={{
                                        ...csConversationRow(isSelected),
                                        display: 'flex',
                                        alignItems: 'center',
                                    ...
                                    >
                                      <div
                                        style={{
                                          ...
                                          height: '28px',
                                          borderRadius: CS.radius,
                                          background: isSelected ? CS.primarySoft : CS.surfaceLow,
                                          display: 'flex',
                                          alignItems: 'center',
                                          justifyContent: 'center',
                                          flexShrink: 0,
                                      ...
                                      >
                                        <Icon svg={ICONS.link} size={14} color={isSelected ? CS.primary : CS.onSurfaceVariant} />
                                      </div>
                                      <div style={{ minWidth: 0, flex: 1 }}>
                                        <div style={{ fontWeight: 600, marginBottom: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: '13px' }}>
                                          {callLink.name || `Call Link ${callLink.roomId.slice(0, 8)}`}
                                        </div>
                                        <div style={{ color: CS.onSurfaceMuted, fontSize: '11px' }}>
                                          {callLink.restrictions === 1 ? 'Admin approval' : 'Open join'}
                                        </div>
                                      </div>
                                    </button>
                                  );
                                ...
                                <div style={{ ...csDisplayMuted(), fontSize: '13px', padding: '8px 0' }}>
                                  No call links yet.
                                </div>
                              )}
                            </div>
                              ...
                              {selectedLaunchCallLink ? (
                                <>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
                                    <div style={{ flex: 1, minWidth: 0 }}>
                                      {editingCallLinkRoomId === selectedLaunchCallLink.roomId ? (
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                          <input
                                            type="text"
                                            value={editingCallLinkName}
                                            onChange={e => setEditingCallLinkName(e.target.value)}
                                            onKeyDown={e => {
                                              if (e.key === 'Enter') {
                                                onUpdateCallLinkName(selectedLaunchCallLink.roomId, editingCallLinkName);
                                          ...
                                          <button
                                            type="button"
                                            onClick={() => setEditingCallLinkRoomId(null)}
                                            style={{
                                              border: 'none',
                                              borderRadius: CS.radiusMd,
                                              padding: '6px 10px',
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                          <div style={{ fontSize: '14px', fontWeight: 700, fontFamily: CS.fontDisplay, color: CS.onSurface }}>
                                            {selectedLaunchCallLink.name || 'Untitled'}
                                          </div>
                                          <button
                                            type="button"
                                            onClick={() => {
                                              setEditingCallLinkRoomId(selectedLaunchCallLink.roomId);
                                              setEditingCallLinkName(selectedLaunchCallLink.name || '');
                                            }}
                                          ...
                                          >
                                            <Icon svg={ICONS.edit} size={13} />
                                          </button>
                                        </div>
                                      )}
                                      <div style={{ fontSize: '11px', color: CS.onSurfaceMuted, marginTop: '2px' }}>
                                        {workspaceCallLinkRestriction} · Exp {workspaceCallLinkExpiration}
                                      </div>
                                    </div>
                                  </div>
                                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                    <button
                                      type="button"
                                      onClick={() => onJoinCallLink(selectedLaunchCallLink.roomId)}
                                      style={{
                                        ...csPrimaryButton(false),
                                        background: CS.gradientSecondary,
                                        boxShadow: '0 2px 10px rgba(27, 123, 110, 0.20)',
                                        display: 'inline-flex',
                                        alignItems: 'center',
                                        gap: '6px',
                                        fontSize: '12.5px',
                                    ...
                                    <button
                                      type="button"
                                      onClick={async () => {
                                        const link = callLinkRootKeyToUrl(selectedLaunchCallLink.rootKey);
                                        if (link) {
                                          await copyCallLink(link);
                                        }
                                      }}
                                      style={{ ...csSecondaryButton(), display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '12.5px', padding: '10px 18px' }}
                                    >
                                    ...
                                    <button
                                      type="button"
                                      onClick={() => {
                                        showShareCallLinkViaSignal(selectedLaunchCallLink, i18n);
                                      }}
                                      style={{ ...csSecondaryButton(), display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '12.5px', padding: '10px 18px' }}
                                    >
                                ...
                                >
                                  <Icon svg={ICONS.link} size={20} color={CS.onSurfaceMuted} style={{ marginBottom: '8px', opacity: 0.4 }} />
                                  <div style={{ fontSize: '12px', color: CS.onSurfaceMuted }}>
                ...
                {activeShellTab === 'chats' ? (
                  <div style={{ display: 'grid', gap: '16px', minHeight: 0 }}>
                    <div>
                      <h2 style={{ ...csWorkspaceTitle(), marginBottom: '4px' }}>Messages</h2>
                      <p style={{ margin: 0, fontSize: '13px', color: CS.onSurfaceMuted }}>
                    ...
                    >
                      <div style={{ ...csCard(), overflow: 'auto', padding: '14px' }}>
                        <div style={{ ...csSectionLabel(), marginBottom: '10px' }}>
                          Conversations
                        </div>
                        <div style={{ display: 'grid', gap: '4px' }}>
                          {launchableConversations.length ? (
                            launchableConversations.map(conversation => {
                              ...
                              return (
                                ...
                                >
                                  <AvatarCircle name={conversation.title} size={28} />
                                  <div style={{ minWidth: 0, flex: 1 }}>
                            ...
                            <div style={{ ...csDisplayMuted(), fontSize: '13px' }}>
                        ...
                        {selectedLaunchConversation ? (
                          ...
                          >
                            <div style={{ minHeight: 0 }}>
                              <SmartTimeline id={selectedLaunchConversation.id} />
                            </div>
                            <div style={{ borderTop: `1px solid ${CS.outlineSubtle}` }}>
                              <SmartCompositionArea id={selectedLaunchConversation.id} />
                            </div>
                          </div>
                        ) : (
                          <div style={{ padding: '32px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, textAlign: 'center' }}>
                            <div
                              style={{
                                width: '44px',
                                height: '44px',
                            ...
                            >
                              <Icon svg={ICONS.chat} size={20} color={CS.onSurfaceMuted} />
                            </div>
                            <div style={{ fontSize: '13px', fontWeight: 600, color: CS.onSurfaceSecondary, marginBottom: '4px' }}>
                              No conversation selected
                            </div>
                            <div style={{ fontSize: '12px', color: CS.onSurfaceMuted }}>
                              Choose a client from the sidebar to view messages.
                            </div>
                          </div>
                        )}
                      </div>
                ...
                {activeShellTab === 'devices' ? (
                  <div style={{ display: 'grid', gap: '24px' }}>
                    <div>
                      <h2 style={{ ...csWorkspaceTitle(), marginBottom: '4px' }}>Devices &amp; Permissions</h2>
                      <p style={{ margin: 0, fontSize: '13px', color: CS.onSurfaceMuted }}>
                        Verify camera, microphone, and audio access before your session.
                      </p>
                    </div>
                        ...
                        <div key={row.label} style={csCard()}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', marginBottom: '12px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                              ...
                              >
                                <Icon svg={DEVICE_ICONS[row.label] ?? ICONS.devices} size={18} color={CS.onSurfaceVariant} />
                              </div>
                              <div style={{ fontWeight: 700, color: CS.onSurface, fontFamily: CS.fontDisplay, fontSize: '15px' }}>
                                {row.label}
                              </div>
                            </div>
                          ...
                          <div
                            style={{
                              ...
                              fontSize: '13px',
                              lineHeight: 1.5,
                              marginBottom: row.action || row.settingsAction ? '14px' : 0,
                            }}
                          >
                            {row.detail}
                          </div>
                          {row.action || row.settingsAction ? (
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '10px' }}>
                              {row.action ? (
                                <button type="button" onClick={row.action} style={{ ...csPrimaryButton(false), display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                              ...
                              {row.settingsAction ? (
                                <button type="button" onClick={row.settingsAction} style={{ ...csSecondaryButton(), display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                    ...
                    {/* Preferred devices */}
                    <div style={{ ...csCard(), display: 'grid', gap: '16px' }}>
                      <div style={{ ...csSectionLabel(), display: 'flex', alignItems: 'center', gap: '6px', marginBottom: 0 }}>
                        <Icon svg={ICONS.system} size={14} color={CS.onSurfaceMuted} />
                        Preferred Devices
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '14px' }}>
                        <div>
                          <div style={{ ...csSectionLabel(), marginBottom: '6px' }}>Camera</div>
                          <select
                            value={selectedCamera ?? ''}
                            onChange={event => {
                              changeIODevice({ selectedDevice: event.target.value, type: CallingDeviceType.CAMERA });
                            }}
                            style={csFormSelect()}
                          >
                        ...
                        <div>
                          <div style={{ ...csSectionLabel(), marginBottom: '6px' }}>Microphone</div>
                          <select
                            value={selectedMicrophone?.index ?? ''}
                            onChange={event => {
                              const nextDevice = availableMicrophones[Number(event.target.value)];
                              if (!nextDevice) return;
                              changeIODevice({ selectedDevice: nextDevice, type: CallingDeviceType.MICROPHONE });
                            }}
                            style={csFormSelect()}
                          >
                            {availableMicrophones.length ? (
                              availableMicrophones.map(device => (
                                <option key={device.index} value={device.index}>{device.name}</option>
                              ))
                            ) : (
                        ...
                        <div>
                          <div style={{ ...csSectionLabel(), marginBottom: '6px' }}>Audio Output</div>
                          <select
                            value={selectedSpeaker?.index ?? ''}
                            onChange={event => {
                              const nextDevice = availableSpeakers[Number(event.target.value)];
                              if (!nextDevice) return;
                              changeIODevice({ selectedDevice: nextDevice, type: CallingDeviceType.SPEAKER });
                            }}
                            style={csFormSelect()}
                          >
                            {availableSpeakers.length ? (
                      ...
                      <button
                        type="button"
                        onClick={() => { void refreshMediaAccess(); }}
                        style={{ ...csSecondaryButton(), justifySelf: 'start', display: 'inline-flex', alignItems: 'center', gap: '6px' }}
                      >
                ...
                {activeShellTab === 'system' ? (
                  <div style={{ display: 'grid', gap: '24px' }}>
                    <div>
                      <h2 style={{ ...csWorkspaceTitle(), marginBottom: '4px' }}>Environment</h2>
                      <p style={{ margin: 0, fontSize: '13px', color: CS.onSurfaceMuted }}>
                    ...
                    >
                      {environmentRows.map(([label, value]) => (
                        <div key={label} style={csCard()}>
                          <div style={{ ...csSectionLabel(), marginBottom: '6px' }}>{label}</div>
                          <div
                            style={{
                              fontSize: '15px',
                              fontWeight: 700,
                    ...
                    <div style={csCard()}>
                      <div style={{ ...csSectionLabel(), marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <Icon svg={ICONS.sparkle} size={14} color={CS.onSurfaceMuted} />
                        Build Pipeline
                      </div>
                      <div style={{ display: 'grid', gap: '10px' }}>
                        {buildPipelineRows.map(([label, value]) => (
                          <div key={label} style={csCardNested()}>
                            <div style={{ ...csSectionLabel(), marginBottom: '4px' }}>{label}</div>
                            <div
                              style={{
                                fontSize: '13px',
                                fontWeight: 600,
                ...
                {activeShellTab === 'console' ? (
                  <div style={{ display: 'grid', gap: '24px' }}>
                    <div>
                      <h2 style={{ ...csWorkspaceTitle(), marginBottom: '4px' }}>Quick Guide</h2>
                      <p style={{ margin: 0, fontSize: '13px', color: CS.onSurfaceMuted }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '16px' }}>
                      {([
                        { icon: ICONS.video, title: 'Sessions', desc: 'Start video or audio calls with individual clients or groups. Monitor participant status in real-time.' },
                        { icon: ICONS.chat, title: 'Messages', desc: 'Send secure messages before, during, or after sessions. Full conversation history available.' },
                        { icon: ICONS.devices, title: 'Devices', desc: 'Verify camera, microphone, and audio permissions. Switch between input/output devices.' },
                        { icon: ICONS.link, title: 'Call Links', desc: 'Create shareable session links with optional admin approval for who can join.' },
                      ] as const).map(guide => (
                        <div key={guide.title} style={csCard()}>
                          ...
                          >
                            <Icon svg={guide.icon} size={18} color={CS.secondary} />
                          </div>
                          <div style={{ fontSize: '14px', fontWeight: 700, color: CS.onSurface, fontFamily: CS.fontDisplay, marginBottom: '6px' }}>
                            {guide.title}
                          </div>
                          <div style={{ fontSize: '12.5px', color: CS.onSurfaceVariant, lineHeight: 1.6 }}>
                            {guide.desc}
                          </div>
                        </div>
                      ))}
                    ...
                    <div style={csCard()}>
                      ...
                      >
                        <strong style={{ color: CS.onSurface, fontFamily: CS.fontDisplay, fontWeight: 700 }}>
                          Session Console
                        </strong>{' '}
                        is your central workspace for managing therapy sessions through Signal.
                        All communication is end-to-end encrypted and private.
                      </div>
                      <div
                        style={{
                          marginTop: '16px',
                      ...
                      >
                        During an active session, this view switches to a live monitoring
                        dashboard with video feeds, participant status, and session controls.
                      </div>
                    </div>
                  </div>
                ) : null}
            ...
            >
              ...
              >
                {shellTabs.map(tab => {
                  ...
                  return (
                    ...
                    >
                      <span
                        style={{ display: 'flex', opacity: isSelected ? 1 : 0.65 }}
              ...
              >
                ...
                >
                  ...
                  >
                    <span style={csStatusDot(getStatusDotColor(activeCall))} />
                    <div style={{ minWidth: 0 }}>
                      <h2
                        style={{
                          margin: 0,
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '4px' }}>
                        ...
                        {activeCall.callMode !== 'Direct' && activeCall.remoteParticipants.length > 0 ? (
                          <span style={{ fontSize: '11px', color: CS.onSurfaceMuted }}>
                            {activeCall.remoteParticipants.length} participant{activeCall.remoteParticipants.length !== 1 ? 's' : ''}
                          </span>
                        ) : null}
                      </div>
                    </div>
                  ...
                  >
                        ...
                        title={
                          activeCall.presentingSource
                            ...
                            : remoteScreenSharingParticipant
                              ? i18n('icu:calling__button--presenting-disabled')
                              : i18n('icu:calling__button--presenting-on')
                        }
                        style={{
                          ...csCallControlButton(Boolean(activeCall.presentingSource)),
                          padding: '8px 16px',
                          fontSize: '12px',
                          fontWeight: 600,
                          opacity:
                    ...
                    <span style={csBadge(
                      activeCall.callMode === 'Direct'
                        ? (activeCall.callState === CallState.Accepted ? 'good' : 'warning')
                        : (activeCall.connectionState === GroupConnectionState.Connected ? 'good' : 'warning')
                    )}>
                      {activeCall.callMode === 'Direct'
                        ? getDirectCallStatus(activeCall)
                        : getGroupCallStatus(activeCall.connectionState, activeCall.joinState)}
                    </span>
                  </div>
                </div>
                ...
                activeCall.callMode === CallModeValue.Direct ? (
                  ...
                  >
                    ...
                    <span style={{ flex: '1 1 220px', fontSize: '12px', lineHeight: 1.45, fontWeight: 600 }}>
                      The client must accept this incoming Signal call on their device.
                      If they were still in the group or call-link session, they may need
                      to leave it or switch to this chat to answer.
                    </span>
                  </div>
                ) : null}
                ...
                {savedMultiPartyContext && onRejoinSavedMultiParty ? (
                  ...
                  >
                    ...
                    <span style={{ flex: '1 1 200px', fontSize: '12px', lineHeight: 1.45 }}>
                      Multi-party session paused: you can return to{' '}
                      <strong>{savedMultiPartyContext.title}</strong> (group or call link).
                    </span>
                    <button
                      type="button"
                    ...
                    >
                      {breakoutBusy ? 'Working…' : 'Rejoin multi-party session'}
                    </button>
                  </div>
                ) : null}
                ...
                >
                  {isGroupCallLobby ? (
                        ...
                        >
                          ...
                          {activeCall.peekedParticipants.length > 0 ? (
                            <div style={csLobbySubtitle()}>
                              ...
                              {activeCall.peekedParticipants.length === 1 ? 'person' : 'people'} in call
                            </div>
                          ) : (
                            <div style={{ ...csLobbySubtitle(), color: 'rgba(255, 255, 255, 0.4)' }}>
                          ...
                          <div style={csCallControlBar()}>
                            <button
                              ...
                              style={csCallControlButton(activeCall.hasLocalAudio)}
                            >
                              <Icon svg={activeCall.hasLocalAudio ? ICONS.mic : ICONS.micOff} size={16} />
                              {activeCall.hasLocalAudio ? 'Mic On' : 'Mic Off'}
                            </button>
                            <button
                              type="button"
                              onClick={onToggleVideo}
                              style={csCallControlButton(activeCall.hasLocalVideo)}
                            >
                              <Icon svg={activeCall.hasLocalVideo ? ICONS.camera : ICONS.cameraOff} size={16} />
                              {activeCall.hasLocalVideo ? 'Camera On' : 'Camera Off'}
                            </button>
                          </div>
                          <button
                            type="button"
                          ...
                          >
                            {activeCall.peekedParticipants.length > 0 ? 'Join Meeting' : 'Start Meeting'}
                          </button>
                        </div>
                      </div>
                    </div>
                  ) : activeCall.callMode === 'Direct' ? (
                    /* ---- Direct call: remote video in main area, local preview as PiP ---- */
                    <div
                      style={{
                        ...csVideoWell(),
                        flex: 1,
                    ...
                    >
                      <div
                        style={{
                          position: 'absolute',
                          inset: 0,
                          background: 'radial-gradient(ellipse at top, rgba(26, 43, 140, 0.18), rgba(10, 10, 14, 0.97) 65%)',
                          overflow: 'hidden',
                        }}
                      >
                        {/* Remote participant video (fills the main area) */}
                        {directConversation ? (
                          <DirectCallRemoteParticipant
                            conversation={directConversation}
                            hasRemoteVideo={activeCall.hasRemoteVideo ?? false}
                            i18n={i18n}
                            isReconnecting={callingIsReconnecting(activeCall)}
                            handleSize={handleDirectVideoSize}
                            setRendererCanvas={setRendererCanvas}
                          />
                        ) : null}

                        {/* Local preview PiP — bottom-right corner */}
                        <div
                          style={{
                            position: 'absolute',
                            bottom: '16px',
                    ...
                    <div style={csSpotlightContainer()}>
                      <div style={csSpotlightMain()}>
                        <TherapistSpotlightStage
                          ...
                          isCallReconnecting={activeCall.connectionState === GroupConnectionState.Reconnecting}
                      ...
                      <div style={{ overflow: 'auto', minHeight: 0 }}>
                        <TherapistVideoTileGrid
                          ...
                          onSpotlightSession={id =>
                            setSpotlightedSessionId(id === spotlightedSessionId ? null : id)
                          }
                    ...
                    >
                      ...
                      >
                        <div ref={localPreviewStageRef} style={{ width: '100%', height: '100%' }} />
                        <div
                          style={{
                            position: 'absolute',
                            top: '16px',
                  ...
                  {isSendingVideo && !shouldShowStageLocalPreview && !isGroupCallLobby ? (
                    <div
                      style={{
                        position: 'fixed',
                        right: '340px',
                    ...
                    >
                      <div ref={localPreviewInsetRef} style={{ width: '100%', height: '100%' }} />
                      <div
                        style={{
                          position: 'absolute',
                          left: '8px',
              ...
              >
                  ...
                  {isSingleSessionInterventionMode ? (
                    <>
                      <button
                        ...
                        onClick={() => {
                          onToggleVideo();
                          setIsSendingVideoToSession(!isSendingVideoToSession);
                        }}
                        style={{
                          border: 'none',
                          borderRadius: CS.radiusMd,
                      ...
                      >
                        <Icon
                          svg={
                            activeCall.hasLocalVideo ? ICONS.camera : ICONS.cameraOff
                          }
                          size={16}
                        />
                        {activeCall.hasLocalVideo ? 'Camera On' : 'Camera Off'}
                ...
                <div style={{ display: 'grid', gap: '6px', marginBottom: '24px' }}>
                  <button
                    ...
                    style={{ ...csSecondaryButton(), display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', fontSize: '12px', padding: '9px 14px', width: '100%' }}
                  >
                    <Icon svg={ICONS.system} size={14} />
                    Advanced Controls
                  </button>
                ...
                <div style={{ display: 'grid', gap: '6px', marginBottom: '24px' }}>
                  ...
                  >
                    <div style={{ color: CS.onSurfaceMuted, marginBottom: '4px' }}>
                      Status
                    </div>
                    <div style={{ color: CS.onSurface, fontWeight: 650, marginBottom: '8px' }}>
                      {remoteSupportState.status.replace('_', ' ')}
                    </div>
                    <div style={{ color: CS.onSurfaceMuted, marginBottom: '2px' }}>
                      Therapist:{' '}
                      {remoteSupportState.therapistSessionId ?? 'No active request'}
                    </div>
                    <div style={{ color: CS.onSurfaceMuted }}>
                      Expires: {formatExpiry(remoteSupportState.expiresAtUnixMs)}
                    </div>
                  </div>
                    ...
                    <>
                      ...
                      <button
                        ...
                        style={{
                          ...
                          cursor: remoteSupportBusyAction !== null ? 'not-allowed' : 'pointer',
                        }}
                      >
                  ...
                  {remoteSupportState.status === 'active' ? (
                    <>
                      <button
                        type="button"
                        onClick={() =>
                          void copyRemoteSupportField('id', remoteSupportState.rustDeskId)
                        }
                        ...
                        style={{
                          ...
                          cursor: remoteSupportState.rustDeskId ? 'pointer' : 'not-allowed',
                        }}
                      >
                      ...
                      <button
                        ...
                        style={{
                          ...
                          fontSize: '12px',
                          padding: '10px 14px',
                          opacity: remoteSupportState.rustDeskPassword ? 1 : 0.6,
                          cursor: remoteSupportState.rustDeskPassword ? 'pointer' : 'not-allowed',
                        }}
                      >
                      ...
                      <button
                        type="button"
                        onClick={() => setShowRemoteSupportPassword(value => !value)}
                        style={{
                          ...csSecondaryButton(),
                          width: '100%',
                          fontSize: '12px',
                      ...
                      >
                        {showRemoteSupportPassword ? 'Hide Password' : 'Reveal Password'}
                      </button>
                      <div style={{ ...csCardNested(), padding: '10px 12px', fontSize: '12px' }}>
                        <div style={{ color: CS.onSurfaceMuted, marginBottom: '2px' }}>
                          ID: {remoteSupportState.rustDeskId ?? 'Not available'}
                        </div>
                        <div style={{ color: CS.onSurfaceMuted }}>
                          Password:{' '}
                          {showRemoteSupportPassword
                            ? (remoteSupportState.rustDeskPassword ?? 'Not available')
                    ...
                    callCues.map(cue => (
                      <div
                        ...
                        style={{
                          padding: '10px 14px',
                          borderRadius: CS.radiusMd,
                          background: cue.tone === 'good' ? CS.successSoft
                            : cue.tone === 'warning' ? CS.warningSoft
                            : cue.tone === 'critical' ? CS.criticalSoft
                            : CS.infoSoft,
                          border: 'none',
                        }}
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '3px' }}>
                          <span style={{ ...csBadge(getToneName(cue.tone)), padding: '2px 8px', fontSize: '10px' }}>
                            {cue.label}
                          </span>
                        </div>
                        <div style={{ fontSize: '12px', lineHeight: 1.5, color: CS.onSurfaceSecondary }}>
                          {cue.detail}
                        </div>
                      </div>
                    ))
                  ) : (
                    <div style={{ fontSize: '12px', color: CS.onSurfaceMuted, padding: '6px 0' }}>
                      All clear — no alerts.
                    </div>
                  )}
                </div>
                ...
                {selectedSession ? (
                  <>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '18px' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: '11px', color: CS.onSurfaceMuted, marginTop: '2px' }}>
                          {selectedSession.subtitle} · {selectedSession.status}
                        </div>
                      </div>
                    </div>

                    {/* Focus + Actions — combined for less sections */}
                    {activeCall.callMode !== CallModeValue.Direct || selectedLiveParticipant ? (
                      <>
                        ...
                        <div style={{ display: 'grid', gap: '6px', marginBottom: '20px' }}>
                          {activeCall.callMode !== CallModeValue.Direct ? (
                            <button
                              type="button"
                              onClick={() =>
                                setSpotlightedSessionId(isSelectedSessionSpotlighted ? null : selectedSession.id)
                              }
                              style={{
                                ...csSecondaryButton(),
                                textAlign: 'left',
                                ...
                                gap: '8px',
                                padding: '10px 14px',
                                background: isSelectedSessionSpotlighted ? CS.primarySoft : CS.card,
                              }}
                            >
                              <Icon svg={ICONS.focus} size={14} color={isSelectedSessionSpotlighted ? CS.primary : CS.onSurfaceVariant} />
                          ...
                          selectedInterventionConversation ? (
                            <button
                              ...
                              style={{
                                ...
                                cursor: breakoutBusy ? 'not-allowed' : 'pointer',
                              }}
                            >
                              <Icon
                                svg={ICONS.pulse}
                          ...
                          {selectedLiveParticipant ? (
                            <>
                              <button
                                type="button"
                                onClick={() => onRemoteMute(selectedLiveParticipant.demuxId)}
                                style={{
                                  border: 'none',
                                  borderRadius: CS.radiusMd,
                                  padding: '10px 14px',
                              ...
                              <button
                                type="button"
                                onClick={() => onRemoveParticipant(selectedLiveParticipant.demuxId)}
                                style={{
                                  border: 'none',
                                  borderRadius: CS.radiusMd,
                                  padding: '10px 14px',
                    ...
                    <div style={{ display: 'grid', gap: '5px', marginBottom: '20px' }}>
                      {selectedSession.rows.map(([label, value]) => (
                        <div key={label} style={{ ...csMetricTile(), padding: '10px 14px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            <div style={{ fontSize: '11px', fontWeight: 600, color: CS.onSurfaceMuted, textTransform: 'uppercase', letterSpacing: '0.04em' }}>{label}</div>
                            <div style={{ fontSize: '13px', fontWeight: 650, color: CS.onSurface, fontFamily: CS.fontDisplay }}>
                              {value}
                            </div>
                          </div>
                        </div>
                    ...
                    {pendingParticipants.length > 0 ? (
                      <>
                        <div style={{ ...csSectionLabel(), marginTop: '20px', marginBottom: '10px' }}>
                          ...
                          {pendingParticipants.map(participant => (
                            ...
                            >
                              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                                <AvatarCircle name={participant.title} size={32} />
                                <div style={{ flex: 1 }}>
                                  <div style={{ fontSize: '13px', fontWeight: 700, color: CS.onSurface, fontFamily: CS.fontDisplay }}>
                                    {participant.title}
                                  </div>
                                  <div style={{ fontSize: '11px', color: CS.onSurfaceMuted }}>
                                    Requesting to join
                                  </div>
                                </div>
                              </div>
                              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '6px' }}>
                                <button
                                  ...
                                  onClick={() => onApprovePendingParticipant(participant.serviceId)}
                                  style={{
                                    border: 'none',
                                    borderRadius: CS.radiusMd,
                                    padding: '8px 12px',
                                    background: participant.serviceId ? CS.successSoft : CS.surfaceHigh,
                                    color: participant.serviceId ? CS.success : CS.onSurfaceMuted,
                                    cursor: participant.serviceId ? 'pointer' : 'not-allowed',
                                    fontWeight: 650,
                                    fontSize: '12px',
                                    fontFamily: CS.fontBody,
                                    display: 'flex',
                                ...
                                <button
                                  type="button"
                                  disabled={!participant.serviceId}
                                  onClick={() => onDenyPendingParticipant(participant.serviceId)}
                                  style={{
                                    border: 'none',
                                    borderRadius: CS.radiusMd,
                                    padding: '8px 12px',
                                    background: participant.serviceId ? CS.criticalSoft : CS.surfaceHigh,
                                    color: participant.serviceId ? CS.critical : CS.onSurfaceMuted,
                                    cursor: participant.serviceId ? 'pointer' : 'not-allowed',
                                    fontWeight: 650,
                                    fontSize: '12px',
                                    fontFamily: CS.fontBody,
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', flex: 1, padding: '32px 16px' }}>
                    {pendingParticipants.length === 0 ? (
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, textAlign: 'center' }}>
                        ...
                        >
                          <Icon svg={ICONS.users} size={18} color={CS.onSurfaceMuted} />
                        </div>
                        <p style={{ margin: 0, fontSize: '13px', fontWeight: 600, color: CS.onSurfaceSecondary, marginBottom: '4px' }}>
                          No participant selected
                        </p>
                        <p style={{ margin: 0, fontSize: '12px', color: CS.onSurfaceMuted }}>
              ...
              <div style={{ ...csMainColumn(), overflow: 'auto' }}>
                ...
                {activeShellTab === 'chats' ? (
                  <div style={{ display: 'grid', gap: '16px', minHeight: 0 }}>
                    ...
                    >
                      <div style={{ ...csCard(), overflow: 'auto', padding: '14px' }}>
                        <div style={{ ...csSectionLabel(), marginBottom: '10px' }}>Conversations</div>
                        <div style={{ display: 'grid', gap: '4px' }}>
                          {launchableConversations.length ? (
                            launchableConversations.map(conversation => {
                              const isSelected = selectedLaunchConversationId === conversation.id;
                              return (
                                <button
                                  ...
                                  type="button"
                                  onClick={() => setSelectedLaunchConversationId(conversation.id)}
                                  style={{ ...csConversationRow(isSelected), display: 'flex', alignItems: 'center', gap: '10px' }}
                                >
                                  <AvatarCircle name={conversation.title} size={28} />
                                  <div style={{ minWidth: 0, flex: 1 }}>
                                    <div style={{ fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: '13px' }}>{conversation.title}</div>
                                    <div style={{ color: CS.onSurfaceMuted, fontSize: '11px' }}>{conversation.type === 'group' ? 'Group' : 'Direct'}</div>
                                  </div>
                                </button>
                              );
                            })
                          ) : (
                            <div style={{ ...csDisplayMuted(), fontSize: '13px' }}>No conversations available.</div>
                          )}
                        </div>
                      </div>
                      <div style={{ ...csCard(), padding: 0, overflow: 'hidden', minHeight: 0, display: 'flex' }}>
                        {selectedLaunchConversation ? (
                          <div style={{ display: 'grid', gridTemplateRows: 'minmax(0, 1fr) auto', minHeight: 0, height: '100%', width: '100%' }}>
                            <div style={{ minHeight: 0 }}><SmartTimeline id={selectedLaunchConversation.id} /></div>
                            <div style={{ borderTop: `1px solid ${CS.outlineSubtle}` }}><SmartCompositionArea id={selectedLaunchConversation.id} /></div>
                          </div>
                        ) : (
                          <div style={{ padding: '32px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, textAlign: 'center' }}>
                            <div style={{ width: '44px', height: '44px', borderRadius: '50%', background: CS.surfaceLow, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '12px' }}>
                              <Icon svg={ICONS.chat} size={20} color={CS.onSurfaceMuted} />
                            </div>
                            <div style={{ fontSize: '13px', fontWeight: 600, color: CS.onSurfaceSecondary, marginBottom: '4px' }}>No conversation selected</div>
                            <div style={{ fontSize: '12px', color: CS.onSurfaceMuted }}>Choose a client from the sidebar to view messages.</div>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                {/* =========== DEVICES TAB (during active call) =========== */}
                {activeShellTab === 'devices' ? (
                  <div style={{ display: 'grid', gap: '24px' }}>
                    <div>
                      <h2 style={{ ...csWorkspaceTitle(), marginBottom: '4px' }}>Devices &amp; Permissions</h2>
                      <p style={{ margin: 0, fontSize: '13px', color: CS.onSurfaceMuted }}>Verify camera, microphone, and audio access.</p>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '16px' }}>
                      {deviceStatusRows.map(row => (
                        <div key={row.label} style={csCard()}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', marginBottom: '12px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                              <div style={{ width: '36px', height: '36px', borderRadius: CS.radiusMd, background: CS.surfaceLow, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                <Icon svg={DEVICE_ICONS[row.label] ?? ICONS.devices} size={18} color={CS.onSurfaceVariant} />
                              </div>
                              <div style={{ fontWeight: 700, color: CS.onSurface, fontFamily: CS.fontDisplay, fontSize: '15px' }}>{row.label}</div>
                            </div>
                            <span style={csBadge(getToneName(row.tone))}>{row.status}</span>
                          </div>
                          <div style={{ ...csDisplayMuted(), fontSize: '13px', lineHeight: 1.5, marginBottom: row.action || row.settingsAction ? '14px' : 0 }}>{row.detail}</div>
                          {row.action || row.settingsAction ? (
                            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '10px' }}>
                              {row.action ? (
                                <button type="button" onClick={row.action} style={{ ...csPrimaryButton(false), display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                                  <Icon svg={ICONS.shield} size={14} />{row.actionLabel}
                                </button>
                              ) : <div />}
                              {row.settingsAction ? (
                                <button type="button" onClick={row.settingsAction} style={{ ...csSecondaryButton(), display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                                  <Icon svg={ICONS.system} size={14} />{row.settingsLabel}
                                </button>
                              ) : null}
                            </div>
                          ) : null}
                    ...
                    <div style={{ ...csCard(), display: 'grid', gap: '16px' }}>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '14px' }}>
                        <div>
                          <div style={{ ...csSectionLabel(), marginBottom: '6px' }}>Camera</div>
                          <select value={selectedCamera ?? ''} onChange={e => { changeIODevice({ selectedDevice: e.target.value, type: CallingDeviceType.CAMERA }); }} style={csFormSelect()}>
                            {availableCameras.length ? availableCameras.map(d => <option key={d.deviceId} value={d.deviceId}>{d.label || 'Camera'}</option>) : <option value="">No cameras detected</option>}
                          </select>
                        </div>
                        <div>
                          <div style={{ ...csSectionLabel(), marginBottom: '6px' }}>Microphone</div>
                          <select value={selectedMicrophone?.index ?? ''} onChange={e => { const d = availableMicrophones[Number(e.target.value)]; if (d) changeIODevice({ selectedDevice: d, type: CallingDeviceType.MICROPHONE }); }} style={csFormSelect()}>
                            {availableMicrophones.length ? availableMicrophones.map(d => <option key={d.index} value={d.index}>{d.name}</option>) : <option value="">No microphones detected</option>}
                          </select>
                        </div>
                        <div>
                          <div style={{ ...csSectionLabel(), marginBottom: '6px' }}>Audio Output</div>
                          <select value={selectedSpeaker?.index ?? ''} onChange={e => { const d = availableSpeakers[Number(e.target.value)]; if (d) changeIODevice({ selectedDevice: d, type: CallingDeviceType.SPEAKER }); }} style={csFormSelect()}>
                            {availableSpeakers.length ? availableSpeakers.map(d => <option key={d.index} value={d.index}>{d.name}</option>) : <option value="">No output devices detected</option>}
                          </select>
                        </div>
                      </div>
                      <button type="button" onClick={() => { void refreshMediaAccess(); }} style={{ ...csSecondaryButton(), justifySelf: 'start', display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                        <Icon svg={ICONS.refresh} size={14} />Refresh Access Status
                      </button>
                    </div>
                  </div>
                ) : null}

                {/* =========== SYSTEM TAB (during active call) =========== */}
                {activeShellTab === 'system' ? (
                  <div style={{ display: 'grid', gap: '24px' }}>
                    <div>
                      <h2 style={{ ...csWorkspaceTitle(), marginBottom: '4px' }}>Environment</h2>
                      <p style={{ margin: 0, fontSize: '13px', color: CS.onSurfaceMuted }}>Technical details about your Signal installation.</p>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
                      {environmentRows.map(([label, value]) => (
                        <div key={label} style={csCard()}>
                          <div style={{ ...csSectionLabel(), marginBottom: '6px' }}>{label}</div>
                          <div style={{ fontSize: '15px', fontWeight: 700, color: CS.onSurface, fontFamily: CS.fontDisplay, letterSpacing: '-0.01em' }}>{value}</div>
                        </div>
                      ))}
                    </div>
                    <div style={csCard()}>
                      <div style={{ ...csSectionLabel(), marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                        <Icon svg={ICONS.sparkle} size={14} color={CS.onSurfaceMuted} />Build Pipeline
                      </div>
                      <div style={{ display: 'grid', gap: '10px' }}>
                        {buildPipelineRows.map(([label, value]) => (
                          <div key={label} style={csCardNested()}>
                            <div style={{ ...csSectionLabel(), marginBottom: '4px' }}>{label}</div>
                            <div style={{ fontSize: '13px', fontWeight: 600, fontFamily: CS.fontMono, color: CS.onSurface, padding: '4px 0' }}>{value}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                {/* =========== HELP TAB (during active call) =========== */}
                {activeShellTab === 'console' ? (
                  <div style={{ display: 'grid', gap: '24px' }}>
                    <div>
                      <h2 style={{ ...csWorkspaceTitle(), marginBottom: '4px' }}>Quick Guide</h2>
                      <p style={{ margin: 0, fontSize: '13px', color: CS.onSurfaceMuted }}>How to use the Session Console effectively.</p>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '16px' }}>
                      {([
                        { icon: ICONS.video, title: 'Sessions', desc: 'Start video or audio calls with individual clients or groups. Monitor participant status in real-time.' },
                        ...
                        <div key={guide.title} style={csCard()}>
                          <div style={{ width: '36px', height: '36px', borderRadius: CS.radiusMd, background: CS.secondarySoft, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '14px' }}>
                            <Icon svg={guide.icon} size={18} color={CS.secondary} />
                          </div>
                          <div style={{ fontSize: '14px', fontWeight: 700, color: CS.onSurface, fontFamily: CS.fontDisplay, marginBottom: '6px' }}>{guide.title}</div>
                          <div style={{ fontSize: '12.5px', color: CS.onSurfaceVariant, lineHeight: 1.6 }}>{guide.desc}</div>
                        </div>
                      ))}
                    </div>
                  </div>
