 ...
 */
/**
 * `clientSessionId` values that match the current call’s remote participants
 * (ACI / service id), for correlating MiMo ingest rows with tiles.
 */
export function getActiveCallMiMoClientSessionIdSet(
  activeCall: ActiveCallType
): ReadonlySet<string> {
  const s = new Set<string>();
  if (activeCall.callMode === CallModeValue.Direct) {
    getDirectRemoteMiMoKeys(activeCall).forEach(k => {
      s.add(k);
    });
  } else {
    activeCall.remoteParticipants.forEach(p => {
      getRemoteParticipantMiMoKeys(p).forEach(k => {
        s.add(k);
      });
    });
  }
